...
export default function CustomDatePicker({ value, onChange, placeholder, className = '', style = {}, label }) {
  ...
  const [viewMonth, setViewMonth] = useState(() => {
    if (value) return parseInt(value.split('-')[1]) - 1 || new Date().getMonth()
      ...
      >
        <div className="flex items-center gap-2">
          {value ? (
            ...
            <span style={{ color: 'var(--text-muted)' }}>{placeholder || label}</span>
          )}
        </div>
        ...
        <div className="absolute top-full mt-1.5 right-0 z-50 rounded-xl border shadow-xl overflow-hidden animate-fade-in"
          ...
          <div className="flex items-center justify-between px-3 py-2 border-t" style={{ borderColor: 'var(--border-color)' }}>
            <button
              ...
              style={{ color: 'var(--accent)', background: 'rgba(245,158,11,0.08)' }}
            >
              Today
            </button>
